...
class PolicyConfirmBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final policyHolderBirth = policyMap[keyPolicyHolderBirth] != null
    ...
    final insuredBirth = policyMap[keyInsuredBirth] != null
    ...
    if (paymentMethod == '월납') {
      displayPaymentPeriod = paymentPeriodRaw.isNotEmpty ? '${paymentPeriodRaw}년' : '';
    } else if (paymentMethod == '일시납') {
      displayPaymentPeriod = '일시납';
    }

    // 계약일/만기일
    final startDate = policyMap[keyStartDate] != null
        ? _toDateFormatted(policyMap[keyStartDate])
        : '';
    final endDate = policyMap[keyEndDate] != null
...
//     final insuredName = policyMap[keyInsured];
//     final insuredSex = policyMap[keyInsuredSex];
//     final insuredBirth = _toDateFormatted(policyMap[keyInsuredBirth]);
//     final productCategory = policyMap[keyProductCategory];
//     final insuranceCompany = policyMap[keyInsuranceCompany];
//     final productName = policyMap[keyProductName];
//     final premium = policyMap[keyPremium];
//     final paymentMethod = policyMap[keyPaymentMethod];
//     final paymentPeriod = policyMap[keyPaymentPeriod];
//     final startDate = _toDateFormatted(policyMap[keyStartDate]);
//     final endDate = _toDateFormatted(policyMap[keyEndDate]);
//
//     return Container(
//       color: bgColor,
//       padding: const EdgeInsets.symmetric(vertical: 25, horizontal: 20),
